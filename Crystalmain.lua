...
combat:Checkbox({
    Label    = "Silent Aim",
    Desc     = "Snaps bullets to the nearest target silently",
    Default  = false,
...
combat:Checkbox({
    Label    = "Triggerbot",
    Desc     = "Auto-fires when crosshair overlaps a hitbox",
    Default  = false,
...
combat:Slider({
    Label    = "Aim FOV",
    Desc     = "Radius of the aimbot field of view (pixels)",
    Min      = 10,
    Max      = 500,
    Default  = 120,
    Step     = 5,
...
combat:Dropdown({
    ...
    Options  = {"Head", "Torso", "RandomPart", "NearestPart"},
    Default  = "Head",
...
combat:Multibox({
    ...
    Desc     = "Select players the aimbot will skip",
    Options  = {"Player1","Player2","Player3","Player4","Player5","Player6"},
    Callback = function(sel)
        local list = {}
        for k in pairs(sel) do table.insert(list,k) end
        print("Ignored:", table.concat(list,", "))
    end,
})
...
combat:Keybind({
    ...
    Callback = function(k) print("Aimbot key:", k.Name) end,
})
...
visuals:Toggle({
    Label    = "Player ESP",
    Desc     = "Draws boxes and names above all players",
    Default  = false,
...
visuals:Toggle({
    Label    = "Tracer Lines",
    Desc     = "Draws lines from screen bottom to each player",
    Default  = false,
...
visuals:Multibox({
    Label    = "ESP Info",
    Desc     = "Choose what information is shown on the ESP",
    Options  = {"Name","Health","Distance","Weapon","Team"},
    Callback = function(sel) print("ESP info updated") end,
})

visuals:Slider({
    Label    = "ESP Max Distance",
    Desc     = "Hide ESP on players beyond this distance (studs)",
    Min      = 50,
    Max      = 2000,
    Default  = 500,
    Step     = 50,
...
visuals:Dropdown({
    Label    = "Cham Style",
    Desc     = "Visual fill style applied to character chams",
    Options  = {"Flat","Wireframe","Neon","Outline"},
    Default  = "Flat",
    Callback = function(v) print("Cham style:", v) end,
})
...
movement:Toggle({
    Label    = "Speed Hack",
    Desc     = "Multiplies your WalkSpeed by the value below",
    Default  = false,
    Callback = function(v) print("SpeedHack:", v) end,
})

movement:Slider({
    Label    = "Walk Speed",
    Desc     = "Target WalkSpeed (default Roblox is 16)",
    Min      = 16,
    Max      = 300,
    Default  = 16,
    Step     = 1,
...
movement:Slider({
    Label    = "Jump Power",
    Desc     = "Target JumpPower (default Roblox is 50)",
    Min      = 50,
    Max      = 500,
    Default  = 50,
    Step     = 5,
...
movement:Toggle({
    Label    = "Fly Mode",
    Desc     = "Enables free flight using WASD + Space/Ctrl",
    Default  = false,
...
movement:Button({
    ...
    Desc     = "Instantly moves your character to the spawn point",
    Callback = function()
        local sp = workspace:FindFirstChild("SpawnLocation")
        if sp and game.Players.LocalPlayer.Character then
            game.Players.LocalPlayer.Character:PivotTo(sp.CFrame + Vector3.new(0,5,0))
        end
    end,
})
...
settings:Keybind({
    Label    = "Toggle UI",
    Desc     = "Show or hide this window with a keypress",
    Default  = Enum.KeyCode.RightShift,
    Callback = function(k) print("UI toggle key:", k.Name) end,
})

settings:Textbox({
    Label       = "Discord Webhook",
    Desc        = "Paste your webhook URL to receive logs",
...
settings:Textbox({
    Label       = "License Key",
    Desc        = "Enter your license key to unlock premium features",
    Placeholder = "XXXX-XXXX-XXXX-XXXX",
    ClearOnFocus= true,
    Callback    = function(v)
        win:Notify("License", v=="" and "Key cleared" or "Key saved!")
    end,
})

settings:Button({
    Label    = "Save Config",
    Desc     = "Writes the current settings to a local file",
    Callback = function()
        win:Notify("Config", "Settings saved successfully", 2.5)
    end,
})

settings:Button({
    Label    = "Reset to Defaults",
    Desc     = "Restores all settings to their original values",
    Callback = function()
        win:Notify("Config", "Reset to defaults", 2)
    end,
})

-- Boot notification
task.delay(1.8, function()
    win:Notify("Crystal UI", "Loaded successfully. Enjoy!", 3)
end)
